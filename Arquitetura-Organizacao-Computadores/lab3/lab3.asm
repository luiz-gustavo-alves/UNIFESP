...
.data
	...
	new_line: .asciiz "\n"
	str_senha: .asciiz "Insira a senha: "
	str_senha_cor: .asciiz "Mensagem Original: "
	str_senha_inc: .asciiz "Senha incorreta."
	
	userInput: .space 256
	str_criptografada: .space 256
	str_decriptografada: .space 256
...
main:
	...
	# IMPRIMIR STRING (str_senha)
	li $v0, 4
	la $a0, str_senha
	syscall
	
	# RECEBE ENTRADA DE DADOS (INTEIRO)
	li $v0, 5		
	syscall
	
	# ARMAZENANDO INPUT DE INTEIRO EM $t0
	move $t0, $v0
	lw $t1, senha
	
	move $a0, $s0
	move $a1, $s1
	la $a2, str_decriptografada
	
	# VERIFICANDO SE A SENHA E VALIDA
	beq $t0, $t1, exibir_msg
...
exibir_msg:
	...
	loop:
		...
		# VERIFICANDO SE O CONTADOR EH IGUAL AO TAMANHO DA STRING
		beq $t0, $a1, str_senha_correta
	
		# VERIFICANDO BYTE (a)
		beq $t1, 97, decripto_a
	
		# VERIFICANDO BYTE (r)
		beq $t1, 114, decripto_r
	
		# VERIFICANDO BYTE (e)
		beq $t1, 101, decripto_e
	
		# VERIFICANDO BYTE (o)
		beq $t1, 111, decripto_o
	
		# VERIFICANDO BYTE (m)
		beq $t1, 109, decripto_m
	
		# VERIFICANDO BYTE (h)
		beq $t1, 104, decripto_h
	
		# VERIFICANDO BYTE (l)
		beq $t1, 108, decripto_l
	
		# VERIFICANDO BYTE (s)
		beq $t1, 115, decripto_s
	
		# VERIFICANDO BYTE (p)
		beq $t1, 112, decripto_p
		
	end_decripto:
		
		sb $t1, str_decriptografada($t0)
...
str_senha_correta:
	...
	move $t0, $a2
	# IMPRIMINDO STRING (str_senha_cor)
	la $a0, str_senha_cor	
	li $v0, 4
	syscall
	
	# IMPRIMINDO STRING (userInput)
	la $a0, str_decriptografada
	li $v0, 4
	syscall
