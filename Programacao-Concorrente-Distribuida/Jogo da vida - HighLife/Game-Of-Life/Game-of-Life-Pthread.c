	...
	for (k = 0; k < NUM_GENERATIONS; k++) {
		...
		for (i = local_start; i < local_end; i++) {
		    ...
		    for (j = 0; j < MATRIX_LENGTH; j++) {
				...
				else if (local_th_data->grid[i][j] == 0 && num_neighbors == 3 ) local_th_data->newGrid[i][j] = 1;
...
void demoGameOfLife(int **grid, int **newGrid, pthread_t *th, thread_data *th_data) {
...
int main() {
	...
	//demoGameOfLife(grid, newGrid, th, th_data);
