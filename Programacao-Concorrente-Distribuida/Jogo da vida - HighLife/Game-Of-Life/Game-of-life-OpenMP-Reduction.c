...
int cellAlive(int **grid) {
	...
	#pragma omp parallel for reduction(+: num_cells) num_threads(NUM_THREADS) private(i, j)
	for (i = 0; i < MATRIX_LENGTH; i++) {
...
void demoHighLife(int **grid, int **newGrid) {
