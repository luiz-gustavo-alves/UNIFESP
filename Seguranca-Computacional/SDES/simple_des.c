...
void simple_des(int num_iterations, char** output) {
	...
	for (i = 0; i < num_iterations; i++) {
		...
		if (op == 'C') {
			output[i] =	encryption(bits, k1, k2);
		}
		else {
			output[i] = decryption(bits, k1, k2);
		}
	}
...
int main() {
	...
	char **output = (char**)malloc(num_iterations * sizeof(char*));

	int i;
	for (i = 0; i < num_iterations; i++) {
		output[i] = (char*)malloc(8 * sizeof(char));
	}

	/* EXECUTA ALGORITMO S-DES */
	simple_des(num_iterations, output);
